...
INSERT INTO cms_users (id, email, display_name, status) VALUES
  ...
  ('33333333-3333-4333-8333-333333333333', 'author@example.com', '작성자', 'ACTIVE') ON CONFLICT (id) DO NOTHING;

INSERT INTO cms_user_roles VALUES
  ('11111111-1111-4111-8111-111111111111', 'ADMIN'),
  ('22222222-2222-4222-8222-222222222222', 'EDITOR'),
  ('33333333-3333-4333-8333-333333333333', 'AUTHOR') ON CONFLICT (user_id, role) DO NOTHING;
...
INSERT INTO cms_taxonomy_terms (id, type, name, slug, description, sort_order) VALUES
  ...
  ('55555555-5555-4555-8555-555555555555', 'TAG', 'CMS', 'cms', 'CMS 운영 태그', 1) ON CONFLICT (id) DO NOTHING;
...
VALUES ('66666666-6666-4666-8666-666666666666', 'ARTICLE', '첫 번째 CMS 소식', 'first-cms-news', 'PUBLISHED',
        ...
        'PUBLIC', '33333333-3333-4333-8333-333333333333', 1, now()) ON CONFLICT (id) DO NOTHING;

INSERT INTO cms_content_categories VALUES ('66666666-6666-4666-8666-666666666666','44444444-4444-4444-8444-444444444444') ON CONFLICT (content_id, term_id) DO NOTHING;
INSERT INTO cms_content_tags     VALUES ('66666666-6666-4666-8666-666666666666','55555555-5555-4555-8555-555555555555') ON CONFLICT (content_id, term_id) DO NOTHING;
...
VALUES ('77777777-7777-4777-8777-777777777777', '66666666-6666-4666-8666-666666666666', 1,
        '첫 번째 CMS 소식', '{}', E'# 첫 번째 CMS 소식\n\n본문을 **Markdown**으로 작성합니다.', '초기 게시', '33333333-3333-4333-8333-333333333333') ON CONFLICT (id) DO NOTHING;

INSERT INTO cms_navigation_menus (id, key, label) VALUES ('88888888-8888-4888-8888-888888888888', 'primary', 'Primary Menu') ON CONFLICT (id) DO NOTHING;
INSERT INTO cms_navigation_items (id, menu_id, label, target_type, target_id, sort_order) VALUES
  ('99999999-9999-4999-8999-999999999999', '88888888-8888-4888-8888-888888888888', '공지', 'CATEGORY', '44444444-4444-4444-8444-444444444444', 1) ON CONFLICT (id) DO NOTHING;
