...
export const dashboardDecorativeAssets = {
  hero: {
    ...
    imageClassName: "pointer-events-none absolute bottom-0 right-0 top-0 z-0 h-full w-[52%] object-cover object-right opacity-80",
    overlayClassName: "pointer-events-none absolute inset-0 z-0 bg-gradient-to-r from-white/75 via-[#eef4ff]/45 to-[#49beff]/5 dark:from-dark/75 dark:via-dark/55 dark:to-primary/10"
  },
  review: {
    cardClassName: "card-box relative h-full min-h-[320px] overflow-hidden",
    backdropClassName: "pointer-events-none absolute bottom-0 right-0 z-0 h-64 w-80 object-contain object-right-bottom opacity-55 dark:opacity-35",
  ...
  return (
      <div className="grid grid-cols-12 gap-6">
        <section className="col-span-12">
          <div className={dashboardDecorativeAssets.hero.className}>
            <img
                src={dashboardHeroOrbits}
                className={dashboardDecorativeAssets.hero.imageClassName}
                alt=""
                aria-hidden="true"
            />
            <div className={dashboardDecorativeAssets.hero.overlayClassName} aria-hidden="true" />

            <div className="relative z-[1] flex items-center gap-4">
              <div className="flex h-[58px] w-[58px] items-center justify-center rounded-2xl bg-primary text-xl font-semibold text-white shadow-md shadow-primary/20">
                H
              </div>
              <div className="flex flex-col gap-1">
                <h1 className="card-title">Dashboard</h1>
                <p className="max-w-xl text-sm leading-6 text-muted-foreground">
                  콘텐츠 운영 현황과 검토 작업을 한 화면에서 확인합니다.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            {metricCards.map((card) => {
              ...
              return (
                  <NavLink
                      key={card.label}
                      className={`w-full rounded-lg border-none p-6 shadow-none transition-all ease-in-out hover:scale-105 ${card.tone}`}
                      to="/content"
                  >
            ...
            <div className="rounded-lg border border-ld bg-white/60 p-4 dark:border-[#333f55] dark:bg-transparent" aria-label="날짜별 콘텐츠 등록 현황 캘린더">
              ...
              <div className="mt-2 grid grid-cols-7 gap-2">
                {publishingCalendar.cells.map((cell) => (
                    <div
                        key={cell.date}
                        className={`min-h-[54px] rounded-md border border-transparent p-2 transition-colors ${calendarCellTone(
                            cell.publishedCount,
                            publishingCalendar.maxPublishedCount
        ...
        <section className="col-span-12 lg:col-span-4">
          <div className={dashboardDecorativeAssets.review.cardClassName}>
            <img
                src={reviewQueueSoftCards}
                className={dashboardDecorativeAssets.review.backdropClassName}
                alt=""
                aria-hidden="true"
            />
              ...
              {reviewQueueEvents.map((event) => (
                  <NavLink key={event.id} className={dashboardDecorativeAssets.review.itemClassName} to="/review">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {event.actor.displayName} · {new Date(event.createdAt).toLocaleString("ko-KR")}
                    </p>
                  </NavLink>
              ))}
            </div>
...
function TimelineEvent({ event }: { event: WorkflowEvent }) {
  return (
      <div className="flex gap-x-3">
        <div className="w-1/4 text-end text-xs text-muted-foreground">
          {new Date(event.createdAt).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}
        </div>
