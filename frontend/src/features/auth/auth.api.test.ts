...
describe("auth api", () => {
  ...
  it("loads and clears the cookie-backed session", async () => {
    await getSession();
    expect(fetchMock).toHaveBeenLastCalledWith("/api/v1/auth/session", expect.objectContaining({ credentials: "include" }));

    fetchMock.mockResolvedValueOnce({ ok: true, status: 204, json: async () => undefined });
    await logout();
    expect(fetchMock).toHaveBeenLastCalledWith("/api/v1/auth/session", expect.objectContaining({ method: "DELETE", credentials: "include" }));
  });
});
