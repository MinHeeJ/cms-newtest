...
export function EmptyState({
  ...
  return (
    <div className="flex min-h-52 flex-col items-center justify-center rounded-md border border-dashed border-ld bg-lightsecondary/40 p-8 text-center dark:border-[#333f55] dark:bg-white/[0.02]">
      <span
        className={`mb-4 flex h-12 w-12 items-center justify-center rounded-full ${toneClass[tone]}`}
      >
...
export function InlineNotice({
  ...
  return (
    <div className={`rounded-md p-4 text-sm leading-6 ${toneClass[tone]}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-3" aria-label="로딩 중">
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div
          key={rowIndex}
          className="grid animate-pulse gap-4 rounded-md border border-ld p-4 dark:border-[#333f55]"
